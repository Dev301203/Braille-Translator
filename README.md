...
The string to translate is passed into the application as an argument at runtime. The application is smart enough to determine if the string given to it is either Braille or English and automatically converts it to the appropriate opposite. 

For the purposes of this challenge Braille is displayed as `O` and `.` where `O` represents a raised dot. The entire English alphabet, the ability to `capitalize` letters, add `spaces`, and the numbers `0` through `9` is included as well. 

After conversion, the translated string is outputted to the terminal. 
